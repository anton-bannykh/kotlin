...
internal fun <T> primitiveArrayConcat(args: Array<Array<T>>): Array<T> {
    var size = 0
    for (i in 0..args.size - 1) {
        size += args[i].size
    }
    val a = args[0]
    val result = js("new a.constructor(size)").unsafeCast<Array<T>>()
    if (a.asDynamic().`$type$` != null) {
        result.asDynamic().`$type$` = a.asDynamic().`$type$`
    }
    size = 0
    for (i in 0..args.size - 1) {
        val arr = args[i]
